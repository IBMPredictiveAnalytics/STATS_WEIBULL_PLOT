...
def weibull(timev, countv=None, etype=None, failcode="F", suspcode="S" , itemname=None, 
            graphds = None, graphfile=None, title="", annotation="", 
            textfile=None, filemode="overwrite"):
...
END IF.
        ...
        if graphfile:
            try:
                spss.Submit(r'''SAVE OUTFILE="%s"''' % graphfile)
            except: # error will be issued by Statistics
                pass
        if graphds is None:
...
def Run(args):
    ...
    oobj = Syntax([
        ...
        Template("GRAPHFILE", subc="SAVE", ktype="literal", var="graphfile"),
